...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
	/*
	 * Hide a sus_map vma from smaps. show_smap() is now single-vma only
	 * (rollup moved to show_smaps_rollup()), so returning early is enough.
	 */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
		/*
		 * Skip accounting for a sus_map vma, but still let it count
		 * toward the [rollup] extent -- matching the pre-split
		 * behaviour where rollup mode skipped the walk, not the vma.
		 */
		if (!(vma->vm_file &&
